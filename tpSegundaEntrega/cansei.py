...
def solve_lp():
    # Recebe as demandas de Belo Horizonte e Juiz de Fora
    demandas = {1: int(bh_entry1.get()), 2: int(bh_entry2.get()), 3: int(bh_entry3.get())}
    demandas_jf = {1: int(jf_entry1.get()), 2: int(jf_entry2.get()), 3: int(jf_entry3.get())}
    
    # Função objetivo
    model.objective = sum(x[i, j] * (fretes[(i, j)] if i <= 3 else fretes_jf[(i, j)])
...
##Restrições de capacidade de oferta
for j in range(1, 4):
...
for i in range(1, 7):
    for j in range(1, 4):
        if x[i, j].value() > 0:
            local = "Belo Horizonte" if i <=3 else "Juiz de Fora"
            if j == 1:
                bateria = "Sony VTC5A"
            elif j == 2:
                bateria = "LG HG2"
                ...
                print("Comprar {:.0f} und da bateria {} do fornecedor {}, enviar para:{}".format(x[i, j].value(), bateria, j, local))
